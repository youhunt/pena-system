        ...
        render: function (data, type, full) {
          return (
            '<div class="form-check font-size-16">\
                        <input class="form-check-input" type="checkbox" id="orderlistIdCheck-' +
            full.id +
            '">\
        ...
        render: function (data, type, full) {
          return (
            '<div class="d-flex gap-3">\
                    <a href="#newOrderModal" data-bs-toggle="modal" class="text-success edit-list" data-edit-id="' +
            full.id +
            '"><i class="mdi mdi-pencil font-size-18"></i></a>\
                    <a href="#removeItemModal" data-bs-toggle="modal" class="text-danger remove-list" data-remove-id="' +
            full.id +
...
function orderDetailShow() {
  Array.from(document.querySelectorAll("#order-list tbody tr")).forEach(
    function (item) {
      item
        .querySelector(".viewdetail-btn")
        .addEventListener("click", function () {
          var orderlistId = item.querySelector(".orderlist-id").innerHTML;
          var customerlistName =
            item.querySelector(".customerlist-name").innerHTML;

          document.querySelector("#orderlist-overview .list-id").innerHTML =
            orderlistId;
          document.querySelector(
            "#orderlist-overview .orderlist-customer"
          ).innerHTML = customerlistName;
        });
    }
  );
}
