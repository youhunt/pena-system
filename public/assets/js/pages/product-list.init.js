...
function loadProductData(datas) {
  ...
  datas.forEach(function (listData, index) {
    var discountLabel = listData.discount
    ...
    document.querySelector("#product-list").innerHTML +=
      ...
      '" alt="" class="img-fluid mx-auto d-block">\
                ...
                <div class="mt-4 d-flex align-items-center">\
                    ...
                    <h5 class="mb-1 text-truncate"><a href="javascript: void(0);" class="text-dark">' +
      listData.productName +
      '</a></h5>\
                    <div class="badge bg-success font-size-11"><i class="bx bxs-star me-1"></i>' +
      listData.rating +
      '</div>\
                    </div>\
  ...
  $("#pricerange").ionRangeSlider({
    ...
    onChange: function (data) {
      ...
      filterDataAll = allProductList.filter(function (product) {
        ...
        return (
          parseFloat(listArray[1]) >= minVal &&
          parseFloat(listArray[1]) <= maxVal
        );
      });
...
document
  ...
  .forEach(function (item) {
    var inputVal = item.querySelector(
      ".form-check-label .rate-value"
    ).innerHTML;
    item
      .querySelector(".form-check-input")
      .addEventListener("change", function () {
